...
public class CompatibleTypes extends ItemStat<StringListData, StringListData> {
    public CompatibleTypes() {
        super("COMPATIBLE_TYPES", Material.COMMAND_BLOCK, "兼容类型",
    ...
    public void whenClicked(@NotNull EditionInventory inv, @NotNull InventoryClickEvent event) {
        if (event.getAction() == InventoryAction.PICKUP_ALL)
            new StatEdition(inv, this).enable("在聊天中输入您要添加的类型的名称");
    ...
    public void whenInput(@NotNull EditionInventory inv, @NotNull String message, Object... info) {
        ...
        inv.getEditedSection().set(getPath(), lore);
        inv.registerTemplateEdition();
        inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + "类型 " + message.toUpperCase() + " 已成功添加");
    }
